...
describe("Saarathi — End-to-End Benchmark Suite", () => {
  ...
  let prompts: { prompt_id: string; user_id: string; request_text: string }[];

  beforeAll(() => {
    store = getStore();
    const promptsPath = path.join(process.cwd(), "../data/benchmark_prompts.json");
  ...
  it("should run B01 (User U01 to Tokyo) successfully", async () => {
    ...
    const tokyoFlight = flightsFromCPT.find(
      (f: FlightRow) => f.destination_city.toLowerCase() === "tokyo" || f.destination === "HND" || f.destination === "NRT"
    );
    expect(tokyoFlight).toBeDefined();
    const destination = tokyoFlight.destination;

    const routeFlights = store.flightsByRoute.get(`${user.home_airport}-${destination}`) ?? [];
    ...
    const counterfactuals = computeCounterfactuals(ranked, pref, routeFlights, { destination });
    ...
    try {
      const explanation = await explain(user.user_id, prompt.request, pref, ranked, alternatives, counterfactuals, confidence);
  ...
  it("should run B03 (User U03 to Bali with Summer flexibility) successfully", async () => {
    ...
    const baliFlight = flightsFromAMS.find(
      (f: FlightRow) => f.destination_city.toLowerCase() === "bali" || f.destination === "DPS"
    );
  ...
  it("should run B04 (User U04 to New York meeting) successfully", async () => {
    ...
    const nyFlight = flightsFromMEL.find(
      (f: FlightRow) => f.destination_city.toLowerCase() === "new york" || f.destination === "JFK" || f.destination === "LGA" || f.destination === "EWR"
    );
  ...
  it("should run B05 (User U05 to Sydney around Holidays) successfully", async () => {
    ...
    const sydFlight = flightsFromLIS.find(
      (f: FlightRow) => f.destination_city.toLowerCase() === "sydney" || f.destination === "SYD"
    );
    ...
    const counterfactuals = computeCounterfactuals(ranked, pref, routeFlights, { destination });
    const layoverCf = counterfactuals.find((cf) => cf.perturbation.kind === "accept_one_stop");
