...
describe("Multi-city Optimizer Module", () => {
  it("should optimize route order and enforce temporal turnaround sanity", async () => {
    ...
    for (let i = 0; i < itinerary.legs.length - 1; i++) {
      const currentArr = new Date(itinerary.legs[i].flight.arrival_utc).getTime();
      const nextDep = new Date(itinerary.legs[i+1].flight.departure_utc).getTime();
    ...
    console.log("[Multi-City Test] Optimized routing order:", itinerary.cities.join(" -> "));
