...
describe('Data Layer — Coercion & Indexing', () => {
  ...
  it('should coerce user rows correctly', () => {
    const raw = {
      ...
      raw_history:
        'always book business, hate connections | redeyes kill my mornings',
    };
  ...
  it('should perform sub-millisecond route lookups', () => {
    ...
    console.log(
      `[Performance] Lookup for route ${route} took ${duration.toFixed(4)}ms (returned ${flights?.length ?? 0} flights).`,
    );
