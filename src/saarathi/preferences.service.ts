...
@Injectable()
export class SaarathiPreferencesService {
  private extractor: FeatureExtractionPipeline | null = null;
  private modelLoading = false;
  private archetypeEmbeddings: Record<string, number[][]> = {};

  async initEmbeddingModel() {
    if (this.extractor || this.modelLoading) return;
    ...
    try {
      ...
      for (const [dimension, phrases] of Object.entries(ARCHETYPES)) {
        this.archetypeEmbeddings[dimension] = [];
        for (const phrase of phrases) {
          const out = await this.extractor(phrase, {
            pooling: 'mean',
            normalize: true,
          });
          this.archetypeEmbeddings[dimension].push(
            Array.from(out.data) as number[],
          );
        }
      }
      console.log(
  ...
  private async findEmbeddingMatch(phrase: string): Promise<{
    dimension: string;
    similarity: number;
    archetype: string;
  } | null> {
    ...
    try {
      ...
      for (const [dimension, embedList] of Object.entries(
        this.archetypeEmbeddings,
      )) {
  ...
  private countHits(text: string, patterns: RegExp[]): number {
  ...
  async inferPreferences(
    ...
    if (!this.extractor && !this.modelLoading) {
    ...
    for (const phraseObj of phrases) {
      ...
      if (this.countHits(phrase, DIRECT_BOOST) > 0) {
        directHits++;
        evidence.push({
      ...
      if (this.countHits(phrase, COST_BOOST) > 0) {
        costHits++;
        evidence.push({
      ...
      if (this.countHits(phrase, CONVENIENCE_BOOST) > 0) {
        convHits++;
        evidence.push({
      ...
      if (this.countHits(phrase, REDEYE_AVOID) > 0) {
        avoidRedeyeHits++;
        evidence.push({
      ...
      if (this.countHits(phrase, REDEYE_OK) > 0) {
        redeyeOkHits++;
        evidence.push({
      ...
      if (!matched && this.extractor) {
        const embedMatch = await this.findEmbeddingMatch(phrase);
        if (embedMatch) {
          evidence.push({
