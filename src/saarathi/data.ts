...
// CSV rows (via papaparse) always yield strings; DB rows (via Prisma) yield
// proper primitives. This coerces either into a definite string without
// risking `String()`'s "[object Object]" fallback on unexpected shapes.
...
export function coerceFlightRow(raw: Record<string, unknown>): FlightRow {
  return {
...
export function coerceUserRow(raw: Record<string, unknown>): UserRow {
  return {
...
export async function initializeStoreFromDb(prisma: PrismaClient) {
  const startMs = Date.now();
  ...
  const durationMs = Date.now() - startMs;
  console.log(
    `[Saarathi Store] Loaded ${usersMap.size} users, ${flights.length} flights, indexed ${flightsByRouteMap.size} routes from DB in ${durationMs}ms.`,
  );
