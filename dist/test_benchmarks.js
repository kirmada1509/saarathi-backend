...
async function run() {
    ...
    for (const s of scenarios) {
        ...
        try {
            ...
            const constraintStage = data.trace?.find((t) => t.id === "constraints");
            ...
            if (mode === "multi-city" && data.itinerary) {
                ...
                winnerStr = legs.map((l) => `${l.from}→${l.to} (${l.flight.airline_name})`).join(", ");
    ...
    for (const r of results) {
        ...
        if (r.constraintsApplied.length > 0) {
            md += `**Constraint Trace:**\n`;
            for (const c of r.constraintsApplied)
                md += `- \`${c}\`\n`;
            md += `\n`;
        }
    ...
    fs_1.default.writeFileSync(path_1.default.join(process.cwd(), "BENCHMARK_REPORT.md"), md, "utf-8");
    console.log(`\n====================================================`);
...
run();
//# sourceMappingURL=test_benchmarks.js.map