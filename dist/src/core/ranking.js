...
function filterAndRank(flights, pref, opts = {}) {
    ...
    const scored = current.map((f, i) => {
        ...
        const score = (costWeight * priceScore +
            ...
            dayBonus;
        const breakdown = {
            price: Math.round(costWeight * priceScore * demandAdj * holidayAdj * 1000) /
                1000,
            direct: Math.round(directWeight * directScore * demandAdj * holidayAdj * 1000) /
                1000,
...
function selectAlternatives(ranked, user) {
    ...
    if (cheapest.flight_id === champion.flight_id) {
        ...
        alternatives.push(makeAlt('cheapest', cheapest, `save $${Math.round(savings)}`, timeLost > 0
            ? `+${timeLost.toFixed(1)}h`
            : timeLost < 0
                ? `-${Math.abs(timeLost).toFixed(1)}h`
                : 'same duration', -savings, cheapest.duration_minutes - champion.duration_minutes));
    }
    ...
    if (fastest.flight_id === champion.flight_id) {
        ...
        alternatives.push(makeAlt('fastest', fastest, `arrive ${timeSaved.toFixed(1)}h earlier`, extraCost > 0
            ? `+$${Math.round(extraCost)}`
            : `save $${Math.round(Math.abs(extraCost))}`, extraCost, -(champion.duration_minutes - fastest.duration_minutes)));
    }
    ...
    if (refundableOptions.length === 0) {
        ...
        if (cheapestRefundable.flight_id === champion.flight_id) {
            ...
            alternatives.push(makeAlt('flexible', cheapestRefundable, 'refundable', extraCost > 0
                ? `+$${Math.round(extraCost)}`
                : `save $${Math.round(Math.abs(extraCost))}`, extraCost, cheapestRefundable.duration_minutes - champion.duration_minutes));
        }
    }
    ...
    if (user.date_flexibility_days === 0) {
        ...
        if (bestShift) {
            ...
            alternatives.push(makeAlt('date_shift', bestShift, savings > 0
                ? `save $${Math.round(savings)}`
                : 'highest score on alternative date', `leave ${getDayName(shiftDate)} instead of ${getDayName(championDate)}`, -savings, bestShift.duration_minutes - champion.duration_minutes));
        }
        else {
